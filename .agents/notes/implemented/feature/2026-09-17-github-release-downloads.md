...
系统提供仅管理员可访问的 Release 元数据接口。接口查询公开 GitHub API 的发布版本和资产，并为浏览器返回经过可配置代理前缀转换的下载地址。查询结果缓存五分钟，浏览器直接从代理下载文件，服务端不转发二进制内容，也不向代理发送 Token。
...
- 生产环境匿名请求 Release 接口返回 `401`。

## Consequences

- 用户可在管理状态下直接选择 Release 版本、发布资产或源码包，并通过代理下载；没有 Release 时仍有明确回退路径。
